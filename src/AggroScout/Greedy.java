...
public class Greedy {
    ...
    static void moveGreedy(RobotController rc, MapLocation tar, int bytecodeleft){
        ...
        try {
            ...
            if (rc.getType() != RobotType.SCOUT) RiE = rc.senseNearbyRobots(R+r+ Constants.SAFETYDISTANCE, rc.getTeam().opponent());
            else RiE = rc.senseNearbyRobots(-1, rc.getTeam().opponent());
            Ti = new TreeInfo[0];
            ...
            boolean shoot = true;
            if (rc.getType() == RobotType.SOLDIER || rc.getType() == RobotType.TANK || rc.getType() == RobotType.SCOUT){
                sortEnemies(rc);
                shoot = false;
            }
    ...
    public static Direction greedyStep(RobotController rc, int bytecodeLeft){
        ...
        for (RobotInfo ri : RiE){
            ...
            float safe = Constants.safetyDistance(rc, ri.getType());
            if (safe > 0){
                l += safe;
